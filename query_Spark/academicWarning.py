...
def academic_warning(**kawargs):
    # Kiểm tra số lượng và giá trị của các tham số truyền vào
    name_department = kawargs.get("name_department")
    command = "docker exec -it spark-master bash -c \"spark-submit --master spark://spark-master:7077 --packages com.datastax.spark:spark-cassandra-connector_2.12:3.2.0 /opt/shared/AcademicWarning.py '{0}'\"".format(name_department)
    print(command)

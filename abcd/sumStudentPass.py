...
def run_spark_job(term):
    # Chạy lệnh spark-submit với mã môn học
    command = f"docker exec -it spark-master bash -c \"spark-submit --master spark://spark-master:7077 --packages com.datastax.spark:spark-cassandra-connector_2.12:3.2.0 /opt/shared/sumStudentPassQuery.py {term}\""
    ...
    try:
        ...
        title_column = ["ID sinh viên", "Tên sinh viên", "Kỳ", "Tổng số môn đã qua"]
